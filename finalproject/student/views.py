...
def homepageview(request, *args, **kwargs):
    ...
    for activebook in activebooks:
        activebook.amountdue = fine(
            activebook.duedate, datetime.datetime.now().date())
        activebook.save()
    actionhistory = History.objects.filter(
        studid=active_user).order_by('-id')[:10]
...
def paymentsuccessview(request, bookid, *args, **kwargs):
    ...
    # print(amount_paid)
    payview = Payment_View(timestamp=timestamp, studid=active_user,
                           bookid=book, amount=amount_paid, transactionid=transaction_id)
    payview.save()
    payhistory = Payment_History(timestamp=timestamp, studid=active_user, bookid=book,
                                 amount=amount_paid, transactionstatus='success', transactionid=transaction_id)
    payhistory.save()
...
def paymentfailureview(request, bookid, *args, **kwargs):
    ...
    payhistory = Payment_History(timestamp=timestamp, studid=active_user, bookid=book,
                                 amount=amount_involved, transactionstatus='failure', transactionid=transaction_id)
    payhistory.save()
